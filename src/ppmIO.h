...
inline void readPPM(const char *filename, float *image, bool isGray = 0)
{
	char buff[16];
	FILE *fp;
	int c, rgb_comp_color;
	unsigned int width, height, channels = isGray ? 1 : 3;
	...
	{
		image[i] = isGray ? (float)tempImage[3 * i] : (float)tempImage[i];
	}
